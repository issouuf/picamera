...
while True:
    ...
    #on récupère les coordonnées du rectangle englobant le contour de la table
    x,y,w,h = cv.boundingRect(countour_table)

    #on dessine le rectangle englobant le contour de la table
    cv.rectangle(frame,(x,y),(x+w,y+h),(0,0,255),2)

    #on affiche les coordonnées x et y 
    #print(f"coordonées de la table: x={x} y={y}")

# Parcourir la liste des contours
    for contour in contours[1:]:  # On commence à 1 car le contour 0 est la table
        # Obtenir le rectangle englobant
        x_obj, y_obj, w_obj, h_obj = cv.boundingRect(contour)

        
        # Ignorer les petits objets
        if w_obj < min_width or h_obj < min_height:
            continue

        # Vérifier si le rectangle englobant est à l'intérieur du rectangle de la table
        if x <= x_obj <= x+w and y <= y_obj <= y+h:
            # Dessiner le rectangle englobant sur l'image
            cv.rectangle(frame, (x_obj, y_obj), (x_obj+w_obj, y_obj+h_obj), (255, 0, 0), 2)

            #on récupere le centre de l'object et on dessine un cercle 
            center_x_obj = x_obj + w_obj//2
            center_y_obj = y_obj + h_obj//2
            cv.circle(frame, (center_x_obj, center_y_obj), 4, (0, 0, 255), -1)

            # Enregistrer les coordonnées du rectangle englobant
            print(f"Object coordinates: x={center_x_obj}, y={center_y_obj}")
